...
var bx,by,bz:integer;
bx2,by2,bz2:integer;
begin
bx:=shape[a].position.x;
 by:=shape[a].position.y;
 bz:=shape[a].position.z;
 shape[a].movetorender(5,5,5,50);
 bx2:=shape[b].position.x;
 by2:=shape[b].position.y;
 bz2:=shape[b].position.z;
 shape[b].movetorender(bx,by,bz,50);
 shape[a].movetorendeer(bx2,by2,bz2,50);
render(1);
end;
begin
for i:= 1 to 4 do
 begin
  for j:=i to 4 do
  begin
   if shape[j].depth> shape[i].depth then
   swap(j,i);
  end;
 end;
end;
